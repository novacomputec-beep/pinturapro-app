...
// API não mudou.
// O teto continua sendo assunto do servidor (422 acima do 2x), que sempre foi a
// fonte da verdade; o campo livre não tenta adivinhá-lo. Só o caso "orçamento
// esgotado" (extensaoMaximaHoras 0/ausente) é tratado aqui, mostrando a mensagem de
// teto atingido em vez de um campo que só poderia falhar.
...
export default function ModalEstenderPrazo({ visivel, unidade = 'horas', extensaoMaximaHoras, mensagemCap, onEstender, onFechar }) {
  ...
  const semOrcamento = (Number(extensaoMaximaHoras) || 0) <= 0
    ...
    <Modal visible={visivel} transparent animationType="fade" statusBarTranslucent onRequestClose={onFechar}>
      <View style={estilos.backdrop}>
        <View style={estilos.card}>
          ...
          >
            <Text style={estilos.fecharTexto}>{semOrcamento ? 'Fechar' : 'Agora não'}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
